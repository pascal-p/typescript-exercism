...
describe('Translate input RNA sequences into proteins', () => {
  ...
  it('Stop codon ends translation', () => {
    expect(ProteinTranslation.proteins('AUGUUUUCUUAAAUG')).toEqual(['Methionine', 'Phenylalanine', 'Serine']);
  });

  it('Invalid codon throws error', () => {
    expect(() => ProteinTranslation.proteins('LOL')).toThrow(new Error('Invalid codon'));
  });

  test('Invalid codon throws error', () => {
    expect(() => ProteinTranslation.proteins('AUGOO')).toThrow(new Error('Invalid codon'));
  });
